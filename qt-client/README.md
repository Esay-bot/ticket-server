...
## 3 分钟演示脚本（录屏用, V1-M2 版；V1-M3 将加入 AI 对话/确认卡片主角）

1. **登录**（30s）：只起 C++ 服务端不起服务层 → 启动 `./ticket-client` → 登录框直接提示
   "请先启动 python -m agent.service" → 起服务层后重新打开 → 输错密码一次（401 人话提示）→ 正确登录。
2. **数据链路**（60s）：登录后车票自动加载 → 点"刷新车票" → 切"我的预约"页签点"刷新我的预约"。
3. **退出重登**（30s）：点"退出登录" → 回到登录框 → 重新登录进入（服务层日志可见旧会话 TCP 关闭）。

...
1. **跑真实评测**：`export DEEPSEEK_API_KEY=sk-...`（CMD 用 set）→ `python -m agent.evaluate`（注意是 `-m` 模块方式，不要 `python agent/evaluate.py`），跑完把成功率/轮数/token 填进简历条目与 README 表格；计划要求"定位并修复至少 2 个失败案例"——失败明细会打印并存 `eval_results/*.json`。

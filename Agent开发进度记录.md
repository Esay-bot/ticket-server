...
1. **跑真实评测**：`export DEEPSEEK_API_KEY=sk-...` → `python -m agent.evaluate`，把成功率/轮数/token 填进简历条目与 README 表格；计划要求"定位并修复至少 2 个失败案例"——失败明细会打印并存 `eval_results/*.json`。
2. **补 M3 真实验收**：CLI 里完整走一遍「查周五去北京的票 → 订倒数第二个 → 确认」，确认模型在真实 Key 下遵守两步确认。
3. **每天开发前**：WSL 里 `sudo service mysql start && cd ~/ser-cli/server && ./ser`；Windows 侧代码改动后同步 `cp -r`（或直接在 WSL 里跑 Python 亦可，两边都能连 6000）。
4. **面试前**：通读 `agent/README.md` 的自测题答案，重点能把"确认门控为什么在代码层""单在途请求约束的由来""粘包半包处理"讲成 60 秒版本。
